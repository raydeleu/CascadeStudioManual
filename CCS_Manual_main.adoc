...
[#img-ccs-fillets]
[caption= Figure 2: ]
image::https://github.com/raydeleu/CascadeStudioManual/blob/main/images/css-car-v2.png[]
...
Let us start with a simple example, constructing a car similar to the one shown in the introduction. To create this car in Cascase Studio you have to start the software, delete all the example code on the left side of the interface and enter the code shown below. Then press [F5] to interpret the code. The result will be shown on the right side. Note that the 3D view can be manipulated with the mouse. Pressing the left mouse button (LMB) while dragging rotates the view, pressing the right mouse button (RMB) while dragging pans or shifts the field of view. Rolling the scroll wheel with the mouse pointer inside the 3D view zooms in and out. 
